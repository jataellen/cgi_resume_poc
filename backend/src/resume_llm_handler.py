    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if "@" in line and not structured_data["contact"]["email"]:
                ...
                and len([c for c in line if c.isdigit()]) >= 7
            ):
            ...
            # Detect sections
            if any(
                keyword in line_lower for keyword in ["summary", "profile", "objective"]
            ):
                ...
                keyword in line_lower for keyword in ["projects", "project experience"]
            ):
                ...
                keyword in line_lower for keyword in ["certification", "certificates"]
            ):
            ...
            if current_section == "profile":
                ...
                if any(
                    keyword in line_lower
                    for keyword in ["company", "position", "title", "role"]
                ):
                    if current_entry:
        ...
        if current_entry and current_section in [
            "experience",
            "projects",
            "volunteer_experience",
        ]:
...
def call_tailored_experience_chain(pdf_text, job_description, role, llm):
    ...
    print(
        f"Looking for experience_schema.json at: {EXPERIENCE_SCHEMA_PATH}"
    )  # Debugging
    ...
    try:
        ...
        raise FileNotFoundError(
            f"Experience schema file not found at {EXPERIENCE_SCHEMA_PATH}"
        )
...
def validate_and_fix_scores(evaluation_result):
    ...
    try:
        ...
        if (
            "ratings_summary" in evaluation_result
            and "section_weights" in evaluation_result
        ):
            weights = evaluation_result["section_weights"]
            ratings = evaluation_result["ratings_summary"]

            calculated_score = sum(
                ratings.get(section, 0) * weight for section, weight in weights.items()
            )
...
def evaluate_resume(
    file_path, job_description=None, target_role=None, evaluation_criteria=None
):
    """
    ...
    try:
        ...
        if not resume_text or len(resume_text.strip()) < 100:
            raise ValueError(
                f"Insufficient text extracted from resume: {len(resume_text)} characters"
            )
        ...
        evaluation_prompt = ENHANCED_EVALUATION_HP.format(
            resume_text=resume_text, job_description_context=job_description_context
        )
...
def resume_stream(
    ...
    if custom_experiences and len(custom_experiences) > 0:
        log(
            f"Adding {len(custom_experiences)} custom experience entries to structured data"
        )
...
def run_resume_evaluation(
    ...
    try:
        ...
        try:
            from src.pdf_evaluation_generator import (
